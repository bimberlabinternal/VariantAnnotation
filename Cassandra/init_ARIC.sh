...
if [[ `isProcessingCompleted` == 0 ]];then
	...
	echo '##INFO=<ID=ARIC,Number=A,Type=Float,Description="This is the ARIC score">' >> $OUTFILE
	echo '#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO' >> $OUTFILE

	zcat $TEMP_FILE | grep -v '#' | grep -v "CHR" | awk -v OFS='\t' ' { print $1, $2, ".", $3, $4, ".", "PASS", "ARIC="$6 } ' >> $OUTFILE
